...
class SerpentineS(Strategy):
    ...
    def step(self, obs):
        ...
        # When there are no units on the screen the dimensions will be mismatched
        if processed_obs.shape == (1, 4):
            processed_obs = self._observations[-1]
            logging.warning(f"There were no units on the screen")
            return NoOp(1)
        ...
        score = delta_score['ally_pineapple'] + delta_score['ally_kills']
...
if __name__ == "__main__":
    ...
    assert agent.TRAINING == False, "Training wasn't diasble correctly"
